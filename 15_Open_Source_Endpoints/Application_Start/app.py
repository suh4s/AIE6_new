...
text_loader = TextLoader("./data/paul_graham_essays.txt") # Ensure this path is correct
...
hf_embeddings = HuggingFaceEndpointEmbeddings(
    model=HF_EMBED_ENDPOINT, # Corrected: Use HF_EMBED_ENDPOINT
    task="feature-extraction",
    huggingfacehub_api_token=HF_TOKEN,
)

async def add_documents_async(vectorstore, documents_to_add): # Renamed 'documents' to 'documents_to_add' to avoid conflict
    await vectorstore.aadd_documents(documents_to_add)
...
hf_llm = HuggingFaceEndpoint(
    endpoint_url=HF_LLM_ENDPOINT, # Corrected: Use HF_LLM_ENDPOINT
    ...
    huggingfacehub_api_token=HF_TOKEN # Added token for consistency if endpoint is protected
)
...
async def start_chat():
    ...
    """
    global hf_retriever # Ensure we are using the globally initialized retriever
    ...
    lcel_rag_chain = (
        ...
        | rag_prompt 
        | hf_llm
        | StrOutputParser() # To ensure the final output is a string
    )
